...
import { metalTextureUrl, woodTextureUrl } from './gameTestAssets'
import type {
  BallKind,
  BallVisualState,
  GameTestStageHandle,
...
const TABLE_CLOTH_COLOR = 0x318751
const RACK_SPACING = BALL_DIAMETER + 2
const RACK_APEX_Y = TABLE.playY + TABLE.playHeight * 0.4
const RACK_CENTER_X = TABLE.playX + TABLE.playWidth / 2
...
function getCappedShotVelocity(fromX: number, fromY: number, toX: number, toY: number) {
  const dx = fromX - toX
  const dy = fromY - toY
  ...
  const speed = Math.min(distance * SHOT_POWER, MAX_SHOT_SPEED)
...
export default forwardRef<GameTestStageHandle>(function GameTestStage(_, ref) {
  const rootRef = useRef<HTMLDivElement | null>(null)
  const handleRef = useRef<GameTestStageHandle | null>(null)

  useImperativeHandle(ref, () => ({
    resetRack: () => handleRef.current?.resetRack(),
    replayBreak: () => handleRef.current?.replayBreak(),
  }))
  ...
  useEffect(() => {
    ...
    const mount = async () => {
      ...
      const [metalTexture, woodTexture] = await Promise.all([
      ...
      function createBallVisual(number: number, x: number, y: number): BallVisualState {
        ...
        textureLayer.alpha = kind === 'cue' ? 0.12 : kind === 'eight' ? 0.2 : 0.24
      ...
      function createBall(number: number, x: number, y: number) {
        const visual = createBallVisual(number, x, y)
      ...
      function updateStatus() {
        ...
        if (dragActive) {
          statusText.text = 'Tirez pour regler le break'
          return
        }

        if (cueRespawnPending) {
          statusText.text = 'La blanche revient en jeu'
          return
        }

        statusText.text = areBallsStopped(balls)
          ? 'Cliquez-glissez sur la blanche pour tirer'
      ...
      function buildRack() {
        const rackCoordinates = getInitialRackCoordinates()
        destroyBallVisuals()
        balls = []

        cueBall = createBall(0, TABLE.cueSpawn.x, TABLE.cueSpawn.y)
        balls.push(cueBall)

        BALL_LAYOUT.forEach((ballNumber, index) => {
          const position = rackCoordinates[index]
          balls.push(createBall(ballNumber, position.x, position.y))
        })
      ...
      function resetBallState() {
        ...
        balls.forEach((ball, index) => {
          ...
          ball.visual.container.visible = true

          if (index === 0) {
            ball.x = TABLE.cueSpawn.x
            ball.y = TABLE.cueSpawn.y
            resetBallTextureMotion(ball)
            return
          }

          const nextPosition = rackCoordinates[index - 1]
      ...
      function handlePointerDown(event: PointerEvent) {
        if (!cueBall || cueBall.isPocketed || !areBallsStopped(balls)) {
          return
        }
      ...
      function handlePointerUp(event: PointerEvent) {
        ...
        const shotVelocity = getCappedShotVelocity(cueBall.x, cueBall.y, point.x, point.y)
        if (!shotVelocity) {
          updateStatus()
          return
        }

        cueBall.vx = shotVelocity.vx
        cueBall.vy = shotVelocity.vy
        updateStatus()
      }

      handleRef.current = {
        resetRack: () => resetBallState(),
        replayBreak: () => {
          resetBallState()
          releaseBreakFrame = window.requestAnimationFrame(() => {
            if (!cueBall) {
              return
            }
    ...
    return () => {
      ...
      window.cancelAnimationFrame(releaseBreakFrame)
  ...
  return <div className="game-test-canvas select-none" ref={rootRef} />
})
