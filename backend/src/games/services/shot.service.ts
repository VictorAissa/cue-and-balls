...
export class ShotService {
  ...
  async processShotResolved(
    ...
    const [game, gamePlayers, gameBalls] = await Promise.all([
    ...
    const nextTurnPlayerId = this.gameRules.resolveNextTurn(
      ...
      dto.pocketedNumbers,
      isFoul,
      alreadyAssigned ? shooter.ballType : null,
      ballTypesAssigned,
    );
    ...
    const shooterBallType: BallType | null = ballTypesAssigned
      ? ballTypesAssigned.solids === shooterId
        ? BallType.SOLIDS
        : BallType.STRIPES
      : shooter.ballType;
  ...
  private async persist(
    ...
    const opponentTurnUpdate = this.prisma.gamePlayer.update({
      ...
      data: {
        ...
        ...(ballTypesAssigned && {
          ballType:
            ballTypesAssigned.solids === opponentId
              ? BallType.SOLIDS
              : BallType.STRIPES,
        }),
      },
