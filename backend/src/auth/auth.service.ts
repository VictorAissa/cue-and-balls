...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<PlayerPublic> {
    ...
    try {
      ...
      if (
        ...
        const code = fields?.includes('email')
          ? 'EMAIL_TAKEN'
          : 'USERNAME_TAKEN';
  ...
  async login(dto: LoginDto): Promise<{ accessToken: string }> {
    ...
    if (!player) throw new UnauthorizedException('USER_DOESNT_EXIST');

    const isPasswordValid = await bcrypt.compare(
      dto.password,
      player.passwordHash,
    );
    if (!isPasswordValid) throw new UnauthorizedException('BAD_LOGIN');
